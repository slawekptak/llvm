...
// Contains the opaque IPC handle obtained from zeMemGetIpcHandleWithProperties
// plus the allocation size required to reconstruct the physical_mem on import.
// ze_ipc_mem_handle_t is a self-contained 64-byte blob that can be sent to
// another process via any IPC mechanism without extra fd-transfer steps.
