...
public:
  ...
  using KernelFastCacheKeyT =
      std::tuple<ur_device_handle_t, /* UR device handle pointer */
  ...
  KernelFastCacheValT tryToGetKernelFast(KeyT &&CacheKey) {
    ...
    if (It != MKernelFastCache.end()) {
      traceKernel("Kernel fetched.", std::get<1>(CacheKey), true);
      return It->second;
    }
  ...
  void saveKernel(KeyT &&CacheKey, ValT &&CacheVal) {
    ...
    // smth in the cache
    traceKernel("Kernel inserted.", std::get<1>(CacheKey), true);
...

          {
            ...
            if (auto FastCacheKeyItr =
              ...
              for (const auto &FastCacheKey : FastCacheKeyItr->second) {
                ...
                traceKernel("Kernel evicted.", std::get<1>(FastCacheKey), true);
              }
